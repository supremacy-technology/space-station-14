...
using Content.Server.ZLevels.Core.Components;
...
public sealed class GameMapMappingZNetworkCommand : LocalizedEntityCommands
{
    [Dependency] private readonly IPrototypeManager _proto = default!;
    [Dependency] private readonly IComponentFactory _compFactory = default!;
    [Dependency] private readonly MapLoaderSystem _mapLoader = default!;
    [Dependency] private readonly ZLevelsSystem _zLevel = default!;
    ...
    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        foreach (var map in _proto.EnumeratePrototypes<GameMapPrototype>())
        {
            ...
            {
                if (station.StationComponentOverrides.TryGetComponent<StationZLevelsComponent>(_compFactory, out var zNetwork))
                {
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        StationZLevelsComponent? zNetwork = null;
        foreach (var station in mapProto.Stations.Values)
        {
            if (station.StationComponentOverrides.TryGetComponent<StationZLevelsComponent>(_compFactory, out var zNet))
            {
                zNetwork = zNet;
                break;
            }
